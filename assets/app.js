...
function populateCountryDashboard(){if(!regionCatalog)return;["export-country","compare-country-left","compare-country-right","study-country-left","study-country-right"].forEach(id=>{const select=$(id),previous=select.value;select.innerHTML="";regionCatalog.countries.forEach(row=>{const option=document.createElement("option");option.value=row.id;option.textContent=row.label;select.appendChild(option);});if(regionCatalog.countries.some(row=>row.id===previous))select.value=previous;else if((id==="compare-country-right"||id==="study-country-right")&&regionCatalog.countries.length>1)select.selectedIndex=1;});}
...
async function drawStudyCountryMaps(result){
  ...
  const left=result.pairs.map((pair,index)=>({idx:pair.left.idx,city_name:pair.left.city_name,color:STUDY_COLORS[index],label:`${index+1} · ${pair.left.city_name} · ${pair.similarity_pct}%`}));
  const right=result.pairs.map((pair,index)=>({idx:pair.right.idx,city_name:pair.right.city_name,color:STUDY_COLORS[index],label:`${index+1} · ${pair.right.city_name} · ${pair.similarity_pct}%`}));
...
function renderCountryStudy(result){
  const pairs=result.pairs.map((pair,index)=>`<li><i style="background:${STUDY_COLORS[index]}">${index+1}</i><span><b>${esc(pair.left.city_name)} ↔ ${esc(pair.right.city_name)}</b><small>${esc(pair.seasonal_alignment)} · ${pair.similarity_pct}%</small></span></li>`).join("");
  $("country-study-output").innerHTML=`<p class="study-caption">${result.sampled?"Cruce amplio: se usó una muestra de ciudades con mayor población para mantener la respuesta ágil.":"Cruce exhaustivo de los pares disponibles."} Los números y colores vinculan cada ciudad en ambos mapas.</p><div class="country-map-pair"><section><canvas id="study-country-map-left" width="980" height="650"></canvas></section><section><canvas id="study-country-map-right" width="980" height="650"></canvas></section></div><ol class="country-pair-list">${pairs}</ol>`;
...
async function runCountryStudy(){
  const leftCountryId=$("study-country-left").value,rightCountryId=$("study-country-right").value,button=$("run-country-study");button.disabled=true;setStatus("Buscando pares climáticos entre países…",true);
  try{const p=getParams(),result=await request("compareCountries",{leftCountryId,rightCountryId,mode:p.mode,season:p.season,weights:p.weights},60000);renderCountryStudy(result);setStatus("Estudio de países listo");}catch(e){setStatus(`No se pudo comparar países: ${e.message}`,true)}finally{button.disabled=false;}
}
